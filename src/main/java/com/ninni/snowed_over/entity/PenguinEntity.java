...
public class PenguinEntity extends Animal {
    ...
    public void tick() {
        ...
        if (this.getEggTicks() == 0 && !this.isNoAi()) {
            setHasEgg(false);
            this.playSound(SnowedOverSoundEvents.ENTITY_PENGUIN_HATCH.get(), 1, 1);
            Optional.ofNullable(SnowedOverEntityTypes.PENGUIN.get().create(level)).ifPresent(entity -> {
                entity.setAge(-24000);
    ...
    protected SoundEvent getAmbientSound() {
        if (this.hasEgg() && this.random.nextInt(4) == 0) return SnowedOverSoundEvents.ENTITY_PENGUIN_EGG_CRACK.get();
        else return SnowedOverSoundEvents.ENTITY_PENGUIN_AMBIENT.get();
    }
    ...
    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource source) { return SnowedOverSoundEvents.ENTITY_PENGUIN_HURT.get(); }
    @Nullable
    @Override
    protected SoundEvent getDeathSound() { return SnowedOverSoundEvents.ENTITY_PENGUIN_DEATH.get(); }
    @Override
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob p_146744_) {
        return SnowedOverEntityTypes.PENGUIN.get().create(world);
    }
